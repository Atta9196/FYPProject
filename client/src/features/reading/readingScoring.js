import { readingScoreToBand } from "./readingConstants";
...
function isFillType(type) {
  return ["sentence-completion", "short-answer", "summary-completion"].includes(type);
}
...
export function scoreReadingObjective(readingSet, answers = {}) {
  ...
  for (const item of items) {
    ...
    if (!given) {
      ...
      isCorrect = acceptable.some((a) => normaliseText(a) === user);
      reason = isCorrect ? "Exact match." : "Answer does not match.";
    } else {
      isCorrect = standardiseTfNg(given) === standardiseTfNg(item.expected);
  ...
  return {
    ...
    band,
    bandScore: band,
    accuracyPercent: totalQuestions ? Math.round((correctCount / totalQuestions) * 100) : 0,
    questionFeedback,
    byType,
    strongQuestionTypes,
    weakQuestionTypes,
    items, // flat scorable units for review UI
  };
}

export function getQuestionsForPassage(readingSet, passageId) {
  return (readingSet?.questions || []).filter((q) => q.passageId === passageId);
}

export function getPassageIndex(passageId) {
  const order = ["passage-1", "passage-2", "passage-3"];
  return order.indexOf(passageId);
}
